...
class TestCloudTracker(unittest.TestCase):
    ...
    def bag_read_all(self, f):
        """Read all messages from the given bag file. Path is relative to test dir."""
        rp = rospkg.RosPack()
    ...
    def _test_track_action(self, goal):
        ...
        client = actionlib.SimpleActionClient('point_cloud_tracker/track', TrackAction)
        client.wait_for_server()
        client.send_goal(goal)
        rospy.sleep(1)

        # Check that tracking has started
        self.assertIsPublishing('/point_cloud_tracker/result/points', PointCloud2 )
        self.assertGreater(len(self.lastMsg().data), 100)

        # Check the pose isn't just an empty default pose
        self.assertIsPublishing('/point_cloud_tracker/result/pose', PoseStamped )
