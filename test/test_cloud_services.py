...
class TestCloudServices(unittest.TestCase):
    ...
    def test_transform_cloud(self):
        ...
        transform_cloud = rospy.ServiceProxy('/cloud_services/transform_cloud', PclTransform)
        resp = None
    ...
    def test_filter_cloud(self):
        ...
        filter_cloud = rospy.ServiceProxy('/cloud_services/filter_cloud', PclFilter)
        resp = None
        ...
        self.assertEqual(resp.point_cloud.header.frame_id, self.lastMsg().header.frame_id)
        # Check that the number of points has been reduced
        self.assertGreater(len(self.lastMsg().data), len(resp.point_cloud.data))
