...
class SimpleMockMetaClass(type):
    """
    Metaclass of mock which contains any attribute. Just to stub any class not needed for error checking.
    """
...
class SimpleMock(object):
    """
    Mock class which contains any attribute. Just to stub any class not needed for error checking.
    """
...
class BufferMock(object):
    ...
    def _is_case_for_robot_frame(target_frame, source_frame, index1, index2):
        frames = ["marker_holder_" + str(index1), "marker_holder_" + str(index2)]
...
class TestConstraintsErrors(TestCase):
    ...
    def test_different_frames_count(self):
        ...
        rospy.get_param = TestConstraintsErrors._params_different_frames_lengths_mock
        try:
            CameraCalibration()
    ...
    def _check_error_log_for_distance_error(message):
        if "Distance between ar_marker_1 and ar_marker_3 is not matching appropriate robot marker distances" == message:
    ...
    def test_different_distances_between_frames(self):
        ...
        tf2_ros.Buffer = BufferMock
        rospy.logerr = TestConstraintsErrors._check_error_log_for_distance_error
        ...
        try:
            ...
            self.assertEqual(
                "Distance between ar_marker_1 and ar_marker_3 is not matching appropriate robot marker distances",
