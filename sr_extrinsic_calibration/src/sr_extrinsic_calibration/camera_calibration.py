...
import rospy
from tf.transformations import superimposition_matrix, quaternion_from_matrix, \
    translation_from_matrix, quaternion_multiply, quaternion_from_euler, vector_norm, euler_from_quaternion
from geometry_msgs.msg import TransformStamped
from sr_extrinsic_calibration.transformation_manager import TransformationManager
...
class CameraCalibration(TransformationManager):
    ...
    def __init__(self):
        ...
        It continuously publishes transformation from base frame to camera frame
        """
    ...
    def _load_parameters(self):
        ...
        """
        camera_pose = rospy.get_param("~initial_tf", [0.0, 0.0, 2.0, 0.0, 0.0, 0.0])
        ...
        q = quaternion_from_euler(camera_pose[3], camera_pose[4], camera_pose[5])
    ...
    def _adjust_camera_position(self):
        ...
        if not numpy.allclose(robot_marker_holders_positions, markers_positions, atol=self._distance_error_tolerance):

            affine_transformation_matrix = superimposition_matrix(markers_positions.T, robot_marker_holders_positions.T)

            translation = translation_from_matrix(affine_transformation_matrix)

            needed_rotation = quaternion_from_matrix(affine_transformation_matrix)

            euler_angles = euler_from_quaternion(needed_rotation)

            apply_translation = not numpy.allclose(translation, numpy.zeros(3), atol=self._distance_error_tolerance)
            apply_rotation = not numpy.allclose(euler_angles, numpy.zeros(3), atol=self._angle_error_tolerance)
            ...
            if apply_rotation:
                ...
                new_rotation = quaternion_multiply(current_rotation, needed_rotation)
    ...
    def print_camera_tf(self):
        """
        Print camera position in form of static transformation node code from launch file
        """
        ...
        rospy.loginfo('<node pkg="tf" type="static_transform_publisher" name="base_to_camera_publisher" ' +
                      'args="%f %f %f %f %f %f %f /%s /%s 100" />', translation.x, translation.y, translation.z,
                      quaternion.x, quaternion.y, quaternion.z, quaternion.w, self._base_frame, self._camera_frame)
